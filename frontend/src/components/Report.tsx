...
export default function Report({ markdown }: { markdown?: string }) {
  ...
  const parsedData = useMemo(() => {
    ...
    // Split by level 2 headers, keeping the header text
    ...
    sections.forEach(section => {
      ...
      if (header.includes('overall risk profile') || header.includes('executive summary')) {
        ...
        // Extract score from header like "MODERATE (0.45)"
        ...
        lines.slice(1).forEach(line => {
          ...
          // Match: [**]Title (Score) - Severity[**]
          // Regex: ^[*]*\s*(.*?)\s*\(([\d.]+)\)\s*-\s*(.*?)\s*[*]*$
          const riskHeaderMatch = trimmedLine.match(/^[*]*\s*(.*?)\s*\(([\d.]+)\)\s*-\s*(.*?)\s*[*]*$/);
          if (riskHeaderMatch) {
            const title = riskHeaderMatch[1].trim();
            const score = riskHeaderMatch[2].trim();
            const severity = riskHeaderMatch[3].trim();
            
            let tagColor = 'bg-blue-100 text-blue-700';
            const sevLower = severity.toLowerCase();
            if (sevLower.includes('critical') || sevLower.includes('highest')) tagColor = 'bg-red-100 text-red-700';
            else if (sevLower.includes('high')) tagColor = 'bg-orange-100 text-orange-700';
            else if (sevLower.includes('moderate-high')) tagColor = 'bg-amber-100 text-amber-700';
            else if (sevLower.includes('moderate') || sevLower.includes('significant')) tagColor = 'bg-blue-100 text-blue-700';
            else if (sevLower.includes('low')) tagColor = 'bg-green-100 text-green-700';

            currentRisk = { title, severity, score, points: [], tagColor };
    ...
    // Fallback for final score if not found in header
    if (finalScore === 0) {
  ...
  const getVerdict = (score: number) => {
    if (score >= 0.7) return { label: 'HIGH RISK', color: 'bg-error shadow-error/10' };
    if (score >= 0.4) return { label: 'MODERATE RISK', color: 'bg-amber-500 shadow-amber-500/10' };
    return { label: 'LOW RISK', color: 'bg-green-500 shadow-green-500/10' };
  };

  const verdict = getVerdict(parsedData.finalScore);
  const scoreColor = parsedData.finalScore >= 0.7 ? 'text-error' : parsedData.finalScore >= 0.4 ? 'text-amber-500' : 'text-green-600';
    ...
    <div className="flex flex-col gap-12">
      ...
      <section className="grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
        ...
        <div className="md:col-span-3 flex justify-end">
          <button className="flex items-center gap-2 px-4 py-2 editorial-card text-primary font-semibold hover:bg-surface-container-low transition-all active:scale-95 text-sm">
          ...
          {parsedData.riskAreas.map((area) => (
            <div key={area.title} className="editorial-card p-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className={`${area.tagColor} px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider`}>
                  {area.severity}
                </span>
        ...
        <div className="editorial-card p-8 bg-surface-container-low/30">
          <ol className="space-y-4 list-decimal list-inside text-on-surface-variant text-sm">
            {parsedData.recommendations.map((rec, idx) => {
              ...
              return <li key={idx} className="pl-2">{rec}</li>;
            })}
          </ol>
        </div>
      </section>
      ...
      <section className="text-center py-12 border-y border-outline-variant/20">
        ...
        <p className={`text-5xl font-extrabold font-headline tracking-tight ${scoreColor}`}>
      ...
      <footer className="flex flex-col md:flex-row justify-between gap-4 text-on-surface-variant">
        <div className="flex items-center gap-2 text-[12px] font-medium">
          ...
          <span>Audit Trail: Report generated on Oct 24, 2023 at 14:42 GMT by AI Engine v4.0</span>
        </div>
        <div className="flex items-center gap-6 text-[12px]">
          <span>Confidential • For internal use only</span>
          <span className="font-bold">ID: CIQ-8829-X</span>
        </div>
      </footer>
    </div>
  );
